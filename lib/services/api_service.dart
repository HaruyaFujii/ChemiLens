...
class ApiService {
  ...
  // 分子構造を検証するメソッド
  static Future<Map<String, dynamic>> validateMolecule({
    required List<Map<String, dynamic>> atoms,
    required List<Map<String, dynamic>> bonds,
  }) async {
    try {
      final response = await _dio.post(
        '$_baseUrl/molecule/validate-structure',
        data: {
          'atoms': atoms,
          'bonds': bonds,
        },
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Failed to validate molecule: ${response.statusMessage}');
      }
    } on DioException catch (e) {
      if (e.response != null && e.response?.data != null) {
        final errorMessage =
  ...
  // 分子式で検索するメソッド
  static Future<Map<String, dynamic>> searchByFormula(String formula) async {
    try {
      final response = await _dio.post(
        '$_baseUrl/molecule/search-formula',
        data: {
          'formula': formula,
        },
      );
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Failed to search formula: ${response.statusMessage}');
      }
    } on DioException catch (e) {
      if (e.response != null && e.response?.data != null) {
        final errorMessage =
  ...
  // 分子式から3D表示用データを取得するメソッド
  static Future<Map<String, dynamic>> get3DDataByFormula(String formula) async {
    try {
      final response = await _dio.post(
        '$_baseUrl/molecule/get-3d-data',
        data: {
          'formula': formula,
        },
      );
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Failed to get 3D data: ${response.statusMessage}');
      }
    } on DioException catch (e) {
      if (e.response != null && e.response?.data != null) {
        final errorMessage =
