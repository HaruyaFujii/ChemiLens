...
async function createSimpleGLB(sdfData: string): Promise<Buffer> {
    ...
    atoms.forEach((atom, atomIndex) => {
        ...
        const radius = data?.radius ?? 0.25;
        const { vertices, indices } = createSphere(atom.x, atom.y, atom.z, radius, 16); // より高品質な球
    ...
    bonds.forEach(bond => {
        ...
        if (atom1 && atom2) {
            ...
            const radius1 = data1?.radius ?? 0.25;
            const radius2 = data2?.radius ?? 0.25;
            ...
            if (bondLength > 0) {
                ...
                // 結合の開始と終了点を原子の表面に調整（少し余裕を持たせる）
                const margin = 0.02; // 原子表面からの余裕
                const startX = atom1.x + nx * (radius1 + margin);
                const startY = atom1.y + ny * (radius1 + margin);
                const startZ = atom1.z + nz * (radius1 + margin);

                const endX = atom2.x - nx * (radius2 + margin);
                const endY = atom2.y - ny * (radius2 + margin);
                const endZ = atom2.z - nz * (radius2 + margin);
                ...
                if (adjustedLength > 0.01) {
                    ...
                    if (bondOrder === 1) {
                        ...
                        // 結合の色（濃いグレー）
                        for (let i = 0; i < vertices.length / 3; i++) {
                            allColors.push(0.4, 0.4, 0.4, 1.0);
                        }
                    } else if (bondOrder === 2) {
                        // 二重結合：平行な2本の結合
                        // 垂直ベクトルを計算
                        ...
                        // 外積で垂直ベクトルを計算
                        const upX = perpY * nz - perpZ * ny;
                        const upY = perpZ * nx - perpX * nz;
                        const upZ = perpX * ny - perpY * nx;
                        const upLength = Math.sqrt(upX * upX + upY * upY + upZ * upZ);
                        const normalizedUpX = upX / upLength;
                        const normalizedUpY = upY / upLength;
                        const normalizedUpZ = upZ / upLength;

                        const offset = bondRadius * 1.5; // 結合間の距離

                        // 1本目の結合
                        const { vertices: v1, indices: i1 } = createCylinder(
                            startX + normalizedUpX * offset, startY + normalizedUpY * offset, startZ + normalizedUpZ * offset,
                            endX + normalizedUpX * offset, endY + normalizedUpY * offset, endZ + normalizedUpZ * offset,
                            bondRadius * 0.8, 6
                        );

                        // 2本目の結合
                        const { vertices: v2, indices: i2 } = createCylinder(
                            startX - normalizedUpX * offset, startY - normalizedUpY * offset, startZ - normalizedUpZ * offset,
                            endX - normalizedUpX * offset, endY - normalizedUpY * offset, endZ - normalizedUpZ * offset,
                            bondRadius * 0.8, 6
                        );

                        const indexOffset1 = allVertices.length / 3;
                        allVertices.push(...v1);
                        allIndices.push(...i1.map(i => i + indexOffset1));

                        const indexOffset2 = allVertices.length / 3;
                        allVertices.push(...v2);
                        allIndices.push(...i2.map(i => i + indexOffset2));

                        // 二重結合の色（少し明るいグレー）
                        for (let i = 0; i < (v1.length + v2.length) / 3; i++) {
                            allColors.push(0.5, 0.5, 0.5, 1.0);
                        }
                    } else if (bondOrder === 3) {
                        // 三重結合：中央1本 + 平行2本
                        // 垂直ベクトルを計算
                        ...
                        const offset = bondRadius * 1.8; // 結合間の距離
                        ...
                        // 3本目の結合
                        const { vertices: v3, indices: i3 } = createCylinder(
                            startX - normalizedUpX * offset, startY - normalizedUpY * offset, startZ - normalizedUpZ * offset,
                            endX - normalizedUpX * offset, endY - normalizedUpY * offset, endZ - normalizedUpZ * offset,
                            bondRadius * 0.6, 6
                        );
                        ...
                        const indexOffset3 = allVertices.length / 3;
                        allVertices.push(...v3);
                        allIndices.push(...i3.map(i => i + indexOffset3));

                        // 三重結合の色（より明るいグレー）
                        for (let i = 0; i < (v1.length + v2.length + v3.length) / 3; i++) {
                            allColors.push(0.6, 0.6, 0.6, 1.0);
                        }
                    }
                }
            }
...
const atomData: { [element: string]: { radius: number; color: [number, number, number] } } = {
    'H': { radius: 0.15, color: [1.0, 1.0, 1.0] }, // 水素 (White) - 小さく
    'B': { radius: 0.35, color: [0.925, 0.439, 0.388] }, // ホウ素 (ピンク)
    'C': { radius: 0.35, color: [0.267, 0.267, 0.267] }, // 炭素 (黒グレー) - 少し小さく
    'N': { radius: 0.32, color: [0.180, 0.525, 0.757] }, // 窒素 (濃い青) - 少し小さく
    'O': { radius: 0.30, color: [1.0, 0.0, 0.0] }, // 酸素 (Red) - 少し小さく
    'F': { radius: 0.28, color: [0.180, 0.800, 0.443] }, // フッ素 (蛍光グリーン) - 少し小さく
    'Na': { radius: 0.5, color: [0.961, 0.690, 0.255] }, // ナトリウム (薄オレンジ) - 少し小さく
    'Mg': { radius: 0.5, color: [0.510, 0.878, 0.667] }, // マグネシウム (薄グリーン)
    'Al': { radius: 0.45, color: [0.741, 0.765, 0.780] }, // アルミニウム (銀色グレー)
    'Si': { radius: 0.42, color: [0.365, 0.427, 0.494] }, // ケイ素 (灰青) - 少し小さく
    'P': { radius: 0.40, color: [0.902, 0.494, 0.133] }, // リン (オレンジ) - 少し小さく
    'S': { radius: 0.38, color: [0.957, 0.816, 0.247] }, // 硫黄 (黄色) - 少し小さく
    'Cl': { radius: 0.40, color: [0.345, 0.839, 0.553] }, // 塩素 (黄緑) - 少し小さく
    'K': { radius: 0.55, color: [0.686, 0.478, 0.773] }, // カリウム (赤紫) - 少し小さく
    'Ca': { radius: 0.55, color: [0.463, 0.843, 0.769] }, // カルシウム (水色グリーン) - 少し小さく
    'Fe': { radius: 0.42, color: [0.878, 0.400, 0.200] }, // 鉄 (オレンジ茶) - 追加
    'I': { radius: 0.5, color: [0.557, 0.267, 0.678] }, // ヨウ素 (紫)
    'DEFAULT': { radius: 0.25, color: [0.8, 0.8, 0.8] }, // Light Gray - 小さく
};
